...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        for stillFrame in stillFrames:
            ...
            # Save in captures folder
            # Look for captures in captures folder and increment capture number
            captures = os.listdir('captures')
            captureNum = 0
            for capture in captures:
                captureNum = max(captureNum, int(capture.split('.')[0]))
            captureNum += 1
            cv2.imwrite('captures/' + str(captureNum) + '.jpg', frame)
